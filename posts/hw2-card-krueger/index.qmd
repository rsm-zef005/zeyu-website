...
In this replication, I focus on the paper’s main employment result: the difference-in-differences estimate comparing the change in New Jersey employment to the change in Pennsylvania employment before and after the 1992 minimum-wage increase. My results closely reproduce the positive DID estimate reported in the original paper.
...
Following the paper’s treatment of permanently closed stores, I set wave 2 employment equal to zero for stores with STATUS2 == 3. For the balanced-sample specification, I instead use only stores with non-missing employment in both waves.
