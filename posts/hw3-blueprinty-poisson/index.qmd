...
```{python}
mean_table = (
    df.groupby("iscustomer", as_index=False)["patents"]
      .mean()
      .assign(group=lambda x: np.where(x["iscustomer"] == 1, "Customer", "Non-customer"))
      [["group", "patents"]]
      .rename(columns={"patents": "Mean patents"})
)
mean_table["Mean patents"] = mean_table["Mean patents"].round(3)
mean_table
```
...
for ax, status, title in zip(axes, [0, 1], ["Non-customers", "Customers"]):
    subset = df.loc[df["iscustomer"] == status, "patents"]
...
fig.suptitle("Patent Counts by Blueprinty Customer Status", y=1.04)
plt.tight_layout()
plt.show()
```
...
region_share.columns = ["Non-customer share", "Customer share"]
(region_share * 100).round(1)
```
...
```{python}
age_table = df.groupby("iscustomer")["age"].agg(["mean", "median", "std", "min", "max"])
age_table.index = ["Non-customer", "Customer"]
age_table.round(2)
```
...
#| echo: false
fig, axes = plt.subplots(1, 2, figsize=(12, 4), sharex=True, sharey=True)
age_bins = np.arange(df["age"].min(), df["age"].max() + 2, 2)

for ax, status, title in zip(axes, [0, 1], ["Non-customers", "Customers"]):
    subset = df.loc[df["iscustomer"] == status, "age"]
    ax.hist(subset, bins=age_bins, edgecolor="white")
    ax.set_title(title)
    ax.set_xlabel("Firm age (years)")
    ax.set_ylabel("Number of firms")

fig.suptitle("Firm Age by Blueprinty Customer Status", y=1.04)
plt.tight_layout()
plt.show()
```
...
Assuming the observations are iid, the joint likelihood is
...
plt.plot(lambda_grid, ll_values, linewidth=2)
plt.axvline(Y.mean(), linestyle="--", linewidth=2)
...
So for the simple Poisson model, the MLE is just the sample mean.
...
X = X.astype(float)
Y = df["patents"].to_numpy().astype(float)
X_mat = X.to_numpy()
...
poisson_regression_loglikelihood(np.zeros(X_mat.shape[1]), Y, X_mat)
```

### Estimating the model by MLE with `scipy.optimize`
...
interpret_table = pd.DataFrame({
    "Variable": glm_fit.params.index,
    "Coefficient": glm_fit.params.values,
    "Std. Error": glm_fit.bse.values,
    "Incidence-rate ratio": np.exp(glm_fit.params.values)
})
interpret_table.round(4)
```
...
1. $X_0$: everyone is set to `iscustomer = 0`
2. $X_1$: everyone is set to `iscustomer = 1`
...
yhat0 = glm_fit.predict(df0)
yhat1 = glm_fit.predict(df1)
...
#| echo: false
plt.figure(figsize=(8, 5))
plt.hist(yhat1 - yhat0, bins=30, edgecolor="white")
plt.axvline((yhat1 - yhat0).mean(), linestyle="--", linewidth=2)
plt.title("Distribution of Predicted Customer Effects Across Firms")
plt.xlabel("Predicted patents under customer status 1 minus 0")
